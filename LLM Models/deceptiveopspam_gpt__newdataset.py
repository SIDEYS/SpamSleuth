# -*- coding: utf-8 -*-
"""DeceptiveOpSpam_GPT__NewDataset.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1dhF76fTkg7suyDrq9GAfVlj1MKq0NTiB
...
testdf = pd.read_excel("/content/fake_review_dataset_GPT4o.xlsx")
...
full_dataset = TextDataset(testdf['text'].tolist(), testdf['label'].tolist(), tokenizer)
...
# Generate and print the classification report for the whole dataset
print("\nClassification Report on the Entire Dataset:")
# Convert the integer labels back to their original string representations for target_names
target_names = label_encoder.inverse_transform(np.unique(all_labels)).astype(str)
print(classification_report(all_labels, all_preds, target_names=target_names))

# Generate and plot the confusion matrix for the whole dataset
cm_full = confusion_matrix(all_labels, all_preds)
plt.figure(figsize=(8, 6))
sns.heatmap(cm_full, annot=True, fmt='d', cmap='Greens', xticklabels=label_encoder.classes_, yticklabels=label_encoder.classes_)
plt.xlabel("Predicted")
plt.ylabel("Actual")
plt.title("Confusion Matrix on the Entire Dataset")
plt.show()

# Generate and plot the normalized confusion matrix for the whole dataset
cm_full_normalized = cm_full.astype('float') / cm_full.sum(axis=1)[:, np.newaxis]
plt.figure(figsize=(8, 6))
sns.heatmap(cm_full_normalized, annot=True, fmt=".2f", cmap="Blues", xticklabels=label_encoder.classes_, yticklabels=label_encoder.classes_)
plt.xlabel("Predicted")
plt.ylabel("Actual")
plt.title("Normalized Confusion Matrix on the Entire Dataset")
plt.show()

# Generate and plot the ROC curve for the whole dataset
all_labels_bin_full = np.eye(num_classes)[all_labels]
all_preds_bin_full = np.eye(num_classes)[all_preds]
plt.figure(figsize=(8, 6))
for i in range(num_classes):
    # Ensure we are using the correct indices for binary labels and predictions
    fpr, tpr, _ = roc_curve(all_labels_bin_full[:, i], np.eye(num_classes)[all_preds][:, i])
    plt.plot(fpr, tpr, label=f"{label_encoder.classes_[i]} (AUC = {auc(fpr, tpr):.2f})")
plt.plot([0, 1], [0, 1], 'k--')
plt.xlabel("False Positive Rate")
plt.ylabel("True Positive Rate")
